...
export function LinkSafetyClient() {
  ...
  // Uses refs to avoid stale closure — the interval always reads the latest
  // link list via a ref that is kept in sync with the fetched data.
  const pollTimerRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const retryCountsRef = useRef<Map<string, number>>(new Map());
  const linksRef = useRef(data?.links ?? []);

  const pendingCount = useMemo(
    () => (data?.links ?? []).filter((l) => l.safetyStatus === "pending" && l.safetyScanId).length,
    [data?.links]
  );
  ...
  useEffect(() => {
    ...
    pollTimerRef.current = setInterval(async () => {
      ...
      let advanced = false;
      const retries = retryCountsRef.current;
      for (const link of stillPending) {
        const count = (retries.get(link.id) ?? 0) + 1;
        retries.set(link.id, count);
        try {
          const res = await fetch(
            `/api/url-scanner/refresh/${link.id}`,
            { method: "POST" }
          );
          if (res.status === 200 || res.status === 503) advanced = true;
        } catch {
          // keep polling
        }
    ...
    <div className="w-full max-w-full space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-border bg-background overflow-hidden">
          ...
          <div className="hidden md:grid grid-cols-[1fr_minmax(190px,auto)_150px_90px_100px] px-4 py-2.5 bg-muted/40 border-b border-border text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">
            <span className="min-w-0">Link</span>
            <span className="min-w-0">Status</span>
            <span className="min-w-0">Last Scanned</span>
            <span className="min-w-0 text-center" title="Open short link">Open</span>
            <span className="min-w-0 text-center">Rescan</span>
          </div>
          ...
          <div className="divide-y divide-border">
            {filteredLinks.map((link) => {
              ...
              return (
                ...
                >
                  <div
                    className="grid grid-cols-[1fr_auto] md:grid-cols-[1fr_minmax(190px,auto)_150px_90px_100px] px-4 py-3 cursor-pointer"
                  ...
                  >
                    ...
                    <div className="hidden md:flex items-center text-xs text-muted-foreground min-w-0">
                      {link.safetyScannedAt
                        ? new Date(link.safetyScannedAt).toLocaleString(
                            undefined,
                    ...
                    <div className="hidden md:flex items-center justify-center">
                      <a
                        ...
                        className="flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground transition-colors duration-200"
