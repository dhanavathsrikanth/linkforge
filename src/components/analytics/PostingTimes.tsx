...
export function PostingTimes({ data, isLoading }: PostingTimesProps) {
  ...
  if (!data || data.buckets.every((b) => b.clicks === 0)) {
    return (
      <div className="rounded-xl border border-slate-200 bg-white p-5">
        <p className="text-sm text-slate-500 text-center py-4">No click data available yet.</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-sm font-semibold text-slate-900">Best Posting Times</h3>
      </div>
        ...
        {data.buckets.map((bucket) => {
          ...
          return (
            <div key={bucket.hour} className="flex flex-col items-center gap-0.5">
              <div
                className={`w-full rounded-sm transition-all ${isPeak ? "ring-1 ring-violet-400" : ""}`}
                style={{
              ...
              />
              <span className={`text-[8px] leading-tight ${isPeak ? "font-bold text-violet-700" : "text-slate-400"}`}>
      ...
      <div className="flex items-center gap-2 rounded-lg bg-violet-50 p-3">
        ...
        <div className="flex-1">
          <p className="text-xs font-semibold text-violet-900">Peak at {data.peak.label}</p>
          <p className="text-xs text-violet-600">{data.peak.clicks} clicks in this hour</p>
        </div>
      </div>

      <p className="mt-3 text-xs text-slate-600 leading-relaxed">{data.recommendation}</p>
    </div>
  );
}
