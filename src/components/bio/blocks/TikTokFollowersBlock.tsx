...
import type { TikTokFollowerData } from "@/lib/gallery/sync";
...
function TikTokLogo() {
  return (
    <svg
      ...
      fill="currentColor"
      viewBox="0 0 512 512"
      className="text-sys-label-primary"
    >
...
export function TikTokFollowersBlock({ block, isEditable, onDelete }: Props) {
  const { data, loading } = useBlockData<TikTokFollowerData>(block.id, isEditable);
        ...
        <div className="flex flex-col justify-between h-full min-h-[120px]">
          ...
          <div className="flex items-center justify-between mt-4">
            <span className="text-xs text-sys-label-secondary">
              {isEditable ? "Connect TikTok in Integrations" : "Not connected"}
            </span>
            <TikTokLogo />
          </div>
        </div>
